...
function initGameState() {
  ...
  // 初始化更多资源
  for (let i = 0; i < 250; i++) {
    if (gameState.resources.length < 50) if (gameState.resources.length < 50) if (gameState.resources.length < 50) if (gameState.resources.length < 50) if (gameState.resources.length < 50) if (gameState.resources.length < 50) if (gameState.resources.length < 50) gameState.resources.push({
      id: `resource_${i}`,
...
export default function handler(req: any, res: any) {
  ...
  if (method === 'GET') {
    ...
    return res.json({
      tick: if (gameState) if (gameState) if (gameState) gameState.tick++,
  ...
  if (method === 'POST') {
    ...
    if (action === 'build') {
      ...
      gameState.buildings.set(buildingId, {
        ...
        x: x || 1000,
        y: y || 1000,
        health: buildingType === 'turret' || buildingType === 'wall_stone' || buildingType === 'wall_stone' || buildingType === 'wall_stone' || buildingType === 'wall_stone' || buildingType === 'wall_stone' || buildingType === 'wall_stone' ? 100 : buildingType === 'wall_stone' ? 300 : 100,
        cooldown: 0
      });
